	...
	new Promise(function(resolve, reject) {
		options.callback = function(data, _setCookie) {
			...
			try {
				...
				console.log(moment().format("YYYY-MM-DD HH:mm:ss") + ' --- 新文章列表：', tempPostIdList);
				//获取新列表，再更新，否则不更新
				...
				console.log(error);
				//发邮件
				mailSend('什么值得买获取【文章列表报错】', `时间: ${moment().format("YYYY-MM-DD HH:mm:ss")}  <br/>错误内容: ${ascii2native(error)}`);
			} finally {}
		}
		...
		options.callback = (data, _setCookie) => {
			try {
				...
				if(data.indexOf('"error_code":0') != -1) {
					console.log(moment().format("YYYY-MM-DD HH:mm:ss") + ' --- 什么值得买 评论成功!!!!');
					//记录评论日志
					let logInfo = {};
					logInfo.cookie = cookieSess.username;
					logInfo.date = moment().format("YYYY-MM-DD HH:mm:ss");
					...
					//发邮件
					mailSend('什么值得买发送【评论报错】' ,`时间: ${moment().format("YYYY-MM-DD HH:mm:ss")}  <br/>用户: ${cookieName} <br/>错误内容: ${ascii2native(data)}`);
				}

			} catch(error) {
				console.log(error);
				//发邮件
				mailSend('什么值得买发送【评论报错】' ,`时间: ${moment().format("YYYY-MM-DD HH:mm:ss")}  <br/>用户: ${cookieName} <br/>错误内容: ${ascii2native(error)}`);
			} finally {}
	...
	new Promise((resolve, reject) => {
		options.callback = (data, _setCookie) => {
			try {
				console.log('data===', data);
				if(data.indexOf('"error_code":0') != -1) {
					console.log(moment().format("YYYY-MM-DD HH:mm:ss") + ' -- 什么值得买 签到成功!!!!');
					//记录签到日志
					let logInfo = {};
					logInfo.cookie = cookieSess.username;
					logInfo.date = moment().format("YYYY-MM-DD HH:mm:ss");
					...
					//发邮件
					mailSend('什么值得买【签到报错】', `时间: ${moment().format("YYYY-MM-DD HH:mm:ss")}  <br/>用户: ${cookieName} <br/>错误内容: ${ascii2native(data)}`);
				}
			} catch(error) {
				console.log(error);
				//发邮件
				mailSend('什么值得买【签到报错】', `时间: ${moment().format("YYYY-MM-DD HH:mm:ss")}  <br/>用户: ${cookieName} <br/>错误内容: ${ascii2native(error)}`);
			} finally {}
		}
		request(options, cookie, referer);
	});
...
//每天5点10执行 签到和评论
schedule.scheduleJob( {rule:'30 10 5 * * *', tz: timeZone}, () => {
	//发现频道 最新
